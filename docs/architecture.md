...
| 11 | Strategic representation | done | A record is plain values and holds no live object, so it pickles and survives a reload. Travel is analytical: elapsed time buys a distance and the distance is spent along the route, so a hull untouched for a week costs what one untouched for a second costs - measured, at 100, 500 and 1000. Materialisation preserves identity and puts her where she sailed to. A hull carrying anything individual, a chest as much as a person, is refused rather than summarised |
...
| 14 | Crew and authority | done | Six posts, a company of divisions and ratings with a quality apiece, and succession down a published order. The skill hook is a seam - `MARITIME_COMPETENCE_POLICY`, answering *well enough* by default so a game that adopts none of it sails as it did - and it now buys four consequences rather than none: the helmsman answers, the lookout picks a faint sail off the skyline, the gunner gets his people through the drill, the carpenter gets more out of the same party. The master and the surgeon are posts that can be filled and succeeded to, and nothing downstream reads their competence yet |
| 15 | Tactical geometry | done | Range, bearing, aspect, closure, arcs and `target`. The pacing question that held it open is answered: pacing is geography and cargo rather than a dial, and its mechanism is seaworthiness confining a small hull, a rating gating command, and escorted routes - all built elsewhere, which is why nothing more is owed here |
...
| 17 | Damage | done | Fire and flooding are built, and they compete: both want hands and both want her slowed, for opposite reasons. Sinking is a process fought with pumps and a fothered sail rather than a threshold, and a foundered hull stops floating with somewhere to go. Repair is built too: her own people mend hull, oars and canvas at sea and a mast only after a fashion, and a yard finishes what they cannot. Hull sections, breaches and occupant transition are built too: a shot goes into bow, waist or quarter by the bearing it came over, a hole is under water when it is lower than she is deep, and what comes through it is the square root of the head - so a laden ship drowns from a hole a light one shrugs off, and a ship already making water settles onto her own wounds. She fills from the bottom and gives up a compartment at half its height, which walks her people up a deck at a time rather than drowning them all at the end |
...
| 19 | Strategic maritime world | done | Nobody authored a shipping lane: a lane is what the ports *are*, so a builder who changes what a place exports changes who sails past it. Merchants on those lanes, fishermen out and home off their own coast, patrols on a beat that closes, and raiders on the richest passage there is - which is where the danger of a cargo comes from, and why choosing a rich one is choosing a dangerous voyage without anybody saying so. Encounters report who is in the offing and stop there; materialising anybody is the game's, because only a game knows whether its players are in a state to be interrupted |
| 20 | Standing orders | done | An order is four plain values - a condition name, an action name and a priority - so it survives a reload where a stored callable would not. The highest priority whose condition holds is the one in force and only one is; the rest are *named* as overridden rather than losing silently. Conditions are re-read every tick rather than latched, so a ship that shortened down for a squall makes sail again when it passes. Both registries merge a game's own over the shipped ones |
| 21 | Passenger services | done | A passenger is an object and a destination, so a game's NPC travellers and its players use the same machinery. Accommodation is derived from her volume; the fare goes onto the ship's purse and comes back off it if she does not arrive, which is what makes a passenger different from cargo. A cycle is validated before she sails - a run that does not close pays for the passage home, or the second sailing starts late and every one after it starts later |
| 22 | Cargo economy | done | Commodities, stowage factors, holds, both capacities, loading and discharge, and the four consequences of load - and now the trade, which `DECISIONS.md` reversed its own deferral on. **Off by default** behind `MARITIME_CARGO_ECONOMY`, so a game with its own economy is untouched. A port has a surplus of what it exports and a shortage of what it imports; price follows from that rather than a table per quay, so the trade routes draw themselves. Worth lives here rather than on `Commodity`, which stays silent about value - and it is authored rather than dressed up as a derivation, because hay and wine stow alike and are not alike |
| 23 | Service expansion | done | Repairs were built with phase 17. A tow is placed astern rather than steered, and costs the tug speed the tick actually steers her by - which is what makes bringing a prize in a decision. Stores are the pacing lever: how far she can go is what she has aboard divided by how many she is feeding, and running out costs morale rather than lives. A berth may want a pilot, off by default so a game that models none is refused nothing. Hulls are priced by the ton burthen, which is how they were actually contracted; the menu itself is in `docs/shipyard.md` |
| 24 | Ownership and customisation | done | She is valued on what she *is*: her worth comes from the dimensions and the condition she has now, so a lengthened hull is worth more and a hammered one less without either being recorded. Three refits, each changing a fact something already reads - copper makes her faster, lengthening makes her carry more and turn worse and swing another boat, doubling makes her slower. Paid from her own purse. A sale moves the hull and reports the price; whose pocket it comes out of is the game's, because this contrib has no people's money in it. Interiors and cosmetics are a game's for the same reason |
| 25 | Wrecks and salvage | done | Wreck lifecycle, drift, survivors, floating cargo and salvage. Tow went to phase 23 beside the tugs, which is the same manoeuvre |
| 26 | The client follows the player | done | An optional graphical interface that swaps in when somebody enters the maritime coordinate system and swaps back when they leave: a chart and instruments at sea, a clickable room map ashore. Keyed on **where the player is**, not on their having crossed a landmark - a dev or a player who teleports must get the right board, which is the bug that produced this row. Written up in `docs/client.md` |
| 27 | Fishing, and what the sea feeds | — | Fish as a thing that is *somewhere*: on banks rather than everywhere, so a fishing ground is a place worth knowing and the boats already working off their own coast in phase 19 are working over something real. Two consumers, and they are why it is worth building at all: a catch is stores, which is the pacing lever in `provisioning` bought back at the cost of time; and a run ashore or a day on the fishing is what a company's morale is mended by, which `crew` can already express and nothing currently offers. Neither invents a skill system - what a given person catches stays a game's |


**What the last ten phases did not bring: commands.**

Every one of them is a model and a seam - `take_in_hand`, `leave_order`, `buy_cargo`,
`take_in_tow`, `take_on_stores`, `salvage`, `book_passage` - and none of them is yet
something a player types. That is deliberate to the extent that the phases were scoped as
models and the contrib has always built the model first; it is *not* deliberate as a
finishing position, because a capability nobody can reach is a capability nobody will find.

The gap is worth stating plainly rather than leaving the table above to imply otherwise: the
twenty-two command modules cover sailing, fighting, mooring, pumping, repairing and building
a hull. They do not yet cover trading a cargo, leaving standing orders, taking a tow, storing
ship, sending her in for a refit, or working a wreck. Those are the next thing to build, and
they are small - the decisions are all made and the results all carry their own refusal
codes.
