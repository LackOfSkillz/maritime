...
from .ballistics import aim_point, hit_chance, time_of_flight
...
class Armed:
    ...
    def serving_seconds(self, mount):
        """
        How long this crew will take over this gun.
        ...
        Notes:
            **Everything that slows a gun crew, in one place.** Their own fear, the wreckage
            they are working round, the hands that are aloft instead of at the guns - and
            the gunner, who has had a competence since posts existed and until now bought
            nothing with it. A good gunner gets his people through the drill faster, which
            is the whole of what a gunner is for.

            On the vessel rather than in `serve`, because every one of these is a fact about
            the *ship*: a gun that reached out for them would have to know what it was
            bolted to.

        """
        from .damage import serving_time
        from .sailing import hands_aloft
        from .stations import GUNNERY

        seconds = serving_time(mount.weapon.reload_time, self.damage, self.hesitation)
        seconds *= 1.0 + hands_aloft(self.sail_plan)

        gunner = self.competence_at(GUNNERY)
        return seconds / gunner if gunner > 0.0 else seconds
